<?php if(!empty($data['rows'])) { ?>
    <table cellpadding="0" cellspacing="0" border="0" class="tbl_repeat">
        ...
        <tbody id="rowsLocal" class="sortRows mar" data-url="<?php echo $data['urlSort']; ?>" >
            <?php foreach($data['rows'] as $item) { ?>
                <tr id="row-<?php echo $item['id']; ?>">
                    <td>+</td>
                    <td>
                        <span class="clickHideShow" data-show="#name-<?php echo $item['id']; ?>"><?php echo Helper::encodeHTML($item['name']); ?></span>
                        <input type="text" name="name-<?php echo $item['id']; ?>" 
                        id="name-<?php echo $item['id']; ?>" class="fld fldList blurUpdateHideShow dn" data-id="<?php echo $item['id']; ?>" 
                        value="<?php echo $item['name']; ?>" />
                    </td>
                    <td>
                        <select name="rate-<?php echo $item['id']; ?>" id="rate-<?php echo $item['id']; ?>" class="fld fldSmall selectRedirect">
                            <option value="">Edit rates</option>
                            <?php if (!empty($data['countries'])) { ?>
                                <?php foreach($data['countries'] as $crow) { ?>
                                    <option value="<?php echo $crow['id']; ?>" 
                                    data-url="<?php echo '/ecommerce/'.$data['objURL']->getCurrent('action', false, 
                    ...
                    <td>
                        <a href="#" data-url="<?php echo '/ecommerce/'.$data['objURL']->getCurrent(array('action', 'id'), false, array('action', 'active', 'id', $item['id'])); ?>" class="clickReplace">
                            <?php echo $item['active'] == 1 ? 'Yes' : 'No'; ?>
                        </a>
                    </td>
                    <td>
                        <a href="#" data-url="<?php echo '/ecommerce/'.$data['objURL']->getCurrent(array('action', 'id'), false, array('action', 'default', 'id', $item['id'])); ?>" 
                    ...
                    <td>
                        <a href="#" 
                        data-url="<?php echo '/ecommerce/'.$data['objURL']->getCurrent(array('action', 'id'), false, array('action', 'duplicate', 'id', $item['id'])); ?>" 
