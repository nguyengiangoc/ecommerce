<?php
    ...
    class Order extends Application {
        ...
        private function _isUserItemValid($user = null) {
        ...
        public function getOrderByToken($token = null) {
            if(!empty($token)) {
                $sql = "SELECT `o`.*, DATE_FORMAT(`o`.`date`, '%D %M %Y %r') AS `date_formatted`, 
                    ...
                    ( SELECT `name` FROM `{$this->_table_4}` WHERE `id` = `o`.`country` ) AS `country_name`,
                    ( SELECT `name` FROM `{$this->_table_4}` WHERE `id` = `o`.`ship_country` ) AS `ship_country_name`
                    FROM `{$this->_table}` `o` WHERE `o`.`token` = ?";
                return $this->_Db->fetchOne($sql, $id);
            }
            return null;
        }
        ...
        public function getOrderItems($id = null) {
            $id = !empty($id) ? $id : $this->id;
            $sql = "SELECT `i`.*, `p`.`name`, (`i`.`price` * `i`.`qty`) AS `price_total` FROM `{$this->_table_2}` `i` LEFT JOIN `{$this->_table_5}` `p` ON `p`.`id` = `i`.`product` WHERE `order` = ?";
            return $this->_Db->fetchAll($sql, $id);
        }
        
        private function _isValidApproval ($array = null, $result = null) {
            return (!empty($array) && !empty($result) && array_key_exists('txn_id', $array) && array_key_exists('payment_status', $array) && array_key_exists('custom', $array));
        }
        ...
        private function _isUpdateOrderValid($id = null, $array = null) {
            ...
            return (!empty($id) && !empty($array) && is_array($array) && array_key_exists('status', $array) && array_key_exists('notes', $array));
        ...
        public function updateOrder($id = null, $array = null) {
            if($this->_isUpdateOrderValid($id, $array)) {
                return $this->_Db->update($this->_table, array('status' => $array['status'], 'notes' => $array['notes']), $id); 
            }
