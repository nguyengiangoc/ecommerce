<?php
    ...
    class Country extends Application {
        ...
        public function getCountries() {
            $sql = "SELECT * FROM `{$this->_table}` WHERE `include` = ? ORDER BY `name` ASC";
            return $this->_Db->fetchAll($sql, 1);
        }
        
        public function getCountry($id = null) {
            if(!empty($id)) {
                $sql = "SELECT * FROM `{$this->_table}` WHERE `id` = ? AND `include` = ?";
        ...
        public function getAllExceptLocal() {
            $sql = "SELECT * FROM `{$this->_table}` WHERE `id` != ? ORDER BY `name` ASC";
