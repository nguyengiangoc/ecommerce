<?php
    ...
    class Shipping extends Application {
        ...
        public function getTypes($local = 0) {
            $sql = "SELECT * FROM `{$this->_table_2}` WHERE `local` = ? ORDER BY `order` ASC";
            return $this->_Db->fetchAll($sql, $local);
        }
        
        private function getLastType($local = 0) {
            $sql = "SELECT `order` FROM `{$this->_table_2}` WHERE `local` = ? ORDER BY `order` DESC LIMIT 0, 1";
        ...
        public function setTypeDefault($id = null, $local = 0) {
            if(!empty($id)) {
                ...
                try {
                    ...
                    $sql = "UPDATE `{$this->_table_2}` SET `default` = ? WHERE `local` = ? AND `id` != ?";
                                                            
                    $this->_Db->executeTransaction($sql, array(0, $local, $id));
                    
                    $sql = "UPDATE `{$this->_table_2}` SET `default` = ? WHERE `local` = ? AND `id` = ?";
                    
                    $this->_Db->executeTransaction($sql, array(1, $local, $id));
        ...
        public function duplicateType($id = null) {
            ...
            if(!empty($type)) {
                ...
                try {
                    ...
                    $this->_Db->insertTransaction($this->_table_2, array('name' => $type['name'].' copy', 'order' => $order, 'local' => $type['local'], 'active' => 0));
                    
                    $newId = $this->_Db->id;
                    
                    $sql = "SELECT * FROM `{$this->_table}` WHERE `type` = ?";
                    $list = $this->_Db->fetchAll($sql, $id);
                    
                    if(!empty($list)) {
                        foreach($list as $row) {
                            $this->_Db->insertTransaction($this->_table, array('type' => $newId, 
                                                            'zone' => $row['zone'], 
                                                            'country' => $row['country'], 
                                                            'weight' => $row['weight'],
                                                            'cost' => $row['cost'],
        ...
        public function isDuplicateLocal($typeId = null, $zoneId = null, $weight = null) {
            if($this->_isTypeZoneWeightNotEmpty($typeId, $zoneId, $weight)) {
                ...
                WHERE `type` = ? AND `zone` = ? AND `weight` = ?";
        ...
        public function isDuplicateInternational($typeId = null, $countryId = null, $weight = null) {
            if($this->_isTypeCountryWeightNotEmpty($typeId, $countryId, $weight)) {
                $sql = "SELECT * FROM `{$this->_table}` 
                        WHERE `type` = ? AND `country` = ? AND `weight` = ?";
        ...
        public function getPostCode($id = null, $zoneId = null) {
            if($this->_isIdZoneNotEmpty($id, $zoneId)) {
                $sql = "SELECT * FROM `{$this->_table_4}` WHERE `id` = ? AND `zone` = ?";
        ...
        public function getPostCodes($zoneId = null) {
            if(!empty($zoneId)) {
                $sql = "SELECT * FROM `{$this->_table_4}` WHERE `zone` = ? ORDER BY `post_code` ASC";
        ...
        public function isDuplicatePostCode($postCode = null) {
            if(!empty($postCode)) {
                $sql = "SELECT * FROM `{$this->_table_4}` WHERE `post_code` = ?";
        ...
        private function _isShippingLocal($user = null) {
            return (($user['same_address'] == 1 && $user['country'] == COUNTRY_LOCAL) || 
        ...
        public function getShippingOptions($user = null) {
            if(!empty($user)) {
                ...
                if($this->_isShippingLocal($user)) {
                    ...
                    $zoneId = $zone['zone'];
                    $sql = "SELECT `t`.*, IF ( ? > ( SELECT MAX(`weight`) FROM `{$this->_table}` 
                                                WHERE `type` = `t`.`id` AND `zone` = ? ),
                                                ( SELECT `cost` FROM `{$this->_table}` 
                                                WHERE `type` = `t`.`id` AND `zone` = ? ORDER BY `weight` DESC LIMIT 0, 1 ),
                                                ( SELECT `cost` FROM `{$this->_table}` 
                                                WHERE `type` = `t`.`id` AND `zone` = ? AND `weight` >= ? 
                                                ORDER BY `weight` ASC LIMIT 0, 1 )
                            ) AS `cost` 
                            FROM `{$this->_table_2}` `t` 
                            WHERE `t`.`local` = ? AND `t`.`active` = ? 
                            ORDER BY `t`.`order` ASC";
                    return $this->_Db->fetchAll($sql, array($weight, $zoneId, $zoneId, $zoneId, $weight, 1, 1)); 
                } else {
        ...
        public function getZone($postCode = null, $strLen = 4) {
            if(!empty($postCode)) {
                ...
                $sql = "SELECT * FROM `{$this->_table_4}` WHERE `post_code` = ? LIMIT 0,1";
                $result = $this->_Db->fetchOne($sql, $postCode);
            ...
            if(!empty($user)) {
                ...
                if($countryId == COUNTRY_LOCAL) {
                    $sql = "SELECT `t`.* FROM `{$this->_table_2}` `t` 
                            WHERE `t`.`local` = ? AND `t`.`active` = ? AND `t`.`default` = ?";
        ...
        public function getShipping($user = null, $shippingId = null) {
            if(!empty($user) && !empty($shippingId)) {
                ...
                if($this->_isShippingLocal($user)) {
                    ...
                    FROM `{$this->_table_2}` `t` 
                    WHERE `t`.`local` = ? AND `t`.`active` = ? AND `t`.`id` = ?";
                    return $this->_Db->fetchOne($sql, array($weight, $zoneId, $zoneId, $zoneId, $weight, 1, 1, $shippingId)); 
                } else {
                    $countryId = $user['same_address'] == 1 ? $user['country'] : $user['ship_country'];
                    $sql = "SELECT `t`.*, IF ( ? > ( SELECT MAX(`weight`) FROM `{$this->_table}` 
                                                WHERE `type` = `t`.`id` AND `country` = ? ),
                                                ( SELECT `cost` FROM `{$this->_table}` 
                                                WHERE `type` = `t`.`id` AND `country` = ? ORDER BY `weight` DESC LIMIT 0, 1 ),
                                                ( SELECT `cost` FROM `{$this->_table}` 
                                                WHERE `type` = `t`.`id` AND `country` = ? AND `weight` >= ? 
                                                ORDER BY `weight` ASC LIMIT 0, 1 )
                    ) AS `cost` 
                    FROM `{$this->_table_2}` `t` 
                    WHERE `t`.`local` = ? AND `t`.`active` = ? AND `t`.`id` = ?";
                    return $this->_Db->fetchOne($sql, array($weight, $countryId, $countryId, $countryId, $weight, 0, 1, $shippingId)); 
                }
            } 
            return null;
        }
