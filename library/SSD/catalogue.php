<?php
    ...
    class Catalogue extends Application {
        ...
        public function getCategoryByIdentity($identity = null) {
            if(!empty($identity)) {
                $sql = "SELECT * FROM `{$this->_table}` WHERE `identity` = ?";
        ...
        public function getProductByIdentity($identity = null) {
            if(!empty($identity)) {
                $sql = "SELECT * FROM `{$this->_table_2}` WHERE `identity` = ?";
                return $this->_Db->fetchOne($sql, $identity);
            }
            return null;
        }
        ...
        public function getProducts($cat = null) {
            $sql = "SELECT * FROM {$this->_table_2} WHERE category = ? ORDER BY date DESC";
        ...
        public function removeProduct($id = null) {
            if(!empty($id)) {
                ...
                if(!empty($product)) {
                    ...
                    $sql = "DELETE FROM `{$this->_table_2}` WHERE `id` = ?";
